  ...
  return (
    <AppShell>
      <PageHeader
        ...
        subtitle="All submitted service forms — view, edit, assign reviewers and manage statuses"
        actions={
          <Button
                ...
                >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-52">
                      <DropdownMenuItem onClick={() => setDetailTarget(sub)}>
            ...
            <div className="space-y-4 pt-1">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Badge className={`text-xs ${SERVICE_TYPE_COLORS[detailTarget.service_type] ?? ""}`}>
                    {SERVICE_TYPE_LABELS[detailTarget.service_type] ?? detailTarget.service_type}
                  </Badge>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Status</p>
                  <Badge className={`text-xs flex w-fit items-center gap-1 ${STATUS_STYLES[detailTarget.status] ?? ""}`}>
                    {STATUS_ICON[detailTarget.status]}
                    {detailTarget.status?.replace(/_/g, " ")}
                  </Badge>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Submitted By</p>
                  <p className="text-sm font-medium">{detailTarget.submitted_name ?? "—"}</p>
                  <p className="text-xs text-muted-foreground">{detailTarget.submitted_email}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Submitted At</p>
                  <p className="text-sm">
                    {new Date(detailTarget.submission_timestamp ?? detailTarget.created_at).toLocaleString("en-US", {
                ...
                {detailTarget.reviewed_by && (
                  <div className="col-span-2">
                    ...
                    <p className="text-sm font-medium text-blue-700 flex items-center gap-1 mt-0.5">
                      ...
                      {detailTarget.reviewed_by?.name ?? `User #${detailTarget.reviewed_by}`}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="border border-border/60 rounded-lg p-3 bg-muted/30">
                ...
                <div className="space-y-2">
                  {Object.entries(detailTarget.form_data ?? {})
                    .filter(([k]) => k !== "attachments" && k !== "agree")
                    .map(([key, val]) => (
                      <div key={key} className="grid grid-cols-[130px_1fr] gap-2 items-start">
                        <span className="text-xs text-muted-foreground">{prettifyKey(key)}</span>
              ...
              {/* Review notes */}
              {detailTarget.review_notes && (
                <div className="border border-blue-200 rounded-lg p-3 bg-blue-50/60">
                  <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-1">
                    Review Notes
                  </p>
                  <p className="text-sm whitespace-pre-wrap">{detailTarget.review_notes}</p>
                </div>
              )}
              <Button id="close-detail-btn" variant="outline" className="w-full" onClick={() => setDetailTarget(null)}>
                Close
              </Button>
            </div>
          )}
