...
export const Route = createFileRoute("/registry/create")({
  head: () => ({ meta: [{ title: "Register Technology — STRP" }] }),
  component: () => (
    <RequireAuth>
      <Page />
    </RequireAuth>
...
function Page() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Technology registered successfully",
      });
  ...
  return (
    <AppShell>
      <PageHeader
        title="Register New Technology"
        subtitle="Add a new technology asset to the central registry"
        actions={
          <Button
            variant="outline"
            size="sm"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            ...
            <Button type="submit" disabled={createMutation.isPending}>
              {createMutation.isPending ? "Registering..." : "Register Technology"}
            </Button>
          </div>
        </form>
      </Card>
