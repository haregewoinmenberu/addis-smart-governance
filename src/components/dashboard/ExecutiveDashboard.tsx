...
/**
 * Executive Dashboard
 * Full governance overview for ITDB administrators — city-wide stats,
 * investment trends, compliance, and (when permitted) the research lifecycle.
 */
export function ExecutiveDashboard() {
  ...
  // The endpoint returns a loosely-shaped analytics payload, so treat as any.
  const { data: researchDataRaw } = useQuery({
    queryKey: ["research-dashboard"],
    queryFn: getResearchProjectDashboard,
    enabled: !!user && !!hasResearchAccess,
  });
  const researchData = researchDataRaw as any;
